...
O propósito deste documento é coletar, analisar e definir as necessidades de alto nível e as principais \emph{features} do ASA\textsuperscript{2}/MIIA. O documento foca nas capacidades necessárias aos \emph{stakeholders} e usuários-alvo para viabilizar a interoperabilidade entre modelos de inteligência artificial (IA) treinados externamente e agentes autônomos executados em simuladores construtivos, com ênfase em cenários nos quais o simulador construtivo está integrado a ambientes de simulação virtual. Os detalhes de como o ASA\textsuperscript{2}/MIIA atende essas necessidades são especificados em artefatos complementares, incluindo casos de uso, \emph{user stories} e documentação suplementar.
...
O ASA\textsuperscript{2}/MIIA não se propõe a:
...
\begin{table}[h]
    \begin{tabular}{| p{0.30\textwidth} | p{0.30\textwidth} | p{0.30\textwidth} |}
        ...
        Utiliza o ASA\textsuperscript{2}/MIIA para estabelecer e manter a interoperabilidade entre modelos de IA e agentes autônomos, assegurando compatibilidade, estabilidade temporal e correto funcionamento no ciclo de simulação \\ \hline
...
O Engenheiro de Simulação atua na configuração, operação e manutenção do simulador construtivo, sendo o principal usuário do ASA\textsuperscript{2}/MIIA. Suas atividades incluem a incorporação e substituição de modelos de inteligência artificial utilizados pelos agentes autônomos, a configuração de parâmetros de execução e a verificação do correto funcionamento do mecanismo no ciclo de simulação.
...
Embora cada uma dessas alternativas apresente vantagens em contextos específicos, nenhuma atende simultaneamente aos requisitos de determinismo, previsibilidade temporal, baixa latência, reprodutibilidade e baixo acoplamento exigidos pelo ambiente de simulação construtiva do ASA\textsuperscript{2}. O ASA\textsuperscript{2}/MIIA posiciona-se como uma solução especializada que busca endereçar essas limitações de forma coerente com o domínio de simulação construtiva.
...
O ASA\textsuperscript{2}/MIIA não substitui o simulador nem os \emph{pipelines} de treinamento de IA; atua como uma camada especializada responsável por garantir propriedades críticas de execução, alinhada às diretrizes institucionais do ASA\textsuperscript{2} para interoperabilidade padronizada de modelos de IA.

No contexto do ASA\textsuperscript{2}, esses mecanismos são empregados prioritariamente em ambientes nos quais o simulador construtivo encontra-se integrado a simuladores virtuais, impondo requisitos adicionais de previsibilidade temporal e estabilidade operacional.
