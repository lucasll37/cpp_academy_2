// =============================================================================
// inference_client.hpp — Public API do cliente AsaMiia
//
// Dois modos de operação, seleccionados automaticamente pela string passada
// ao construtor:
//
//   InferenceClient client("localhost:50052");   // modo gRPC (comportamento original)
//   InferenceClient client("inprocess");         // modo in-process (sem rede)
//
// Strings reconhecidas como in-process: "inprocess", "in_process", "local".
// Qualquer outra string é tratada como endereço gRPC ("host:porta").
//
// Todas as assinaturas públicas são idênticas nos dois modos.
// =============================================================================
...
// ============================================
// Tipos de retorno públicos (inalterados)
// ============================================
...
struct ValidationResult {
    ...
    // Schema preview — populated when the backend can introspect without a
    // full load (mirrors ValidateModelResponse.inputs / .outputs in the proto).
...
// ============================================
// Interface interna (detalhe de implementação)
// ============================================

/// Interface que abstrai o transporte: gRPC remoto ou engine local.
/// Não faz parte da API pública; declarada no header apenas para permitir
/// que o destrutor do InferenceClient seja definido no .cpp (onde os tipos
/// concretos são completos).
...
public:
    /// Constrói um cliente.
    ///
    /// @param target  Endereço gRPC ("host:porta") ou uma das strings
    ///                especiais que activam o modo in-process:
    ///                  - "inprocess"
    ///                  - "in_process"
    ///                  - "local"
