// NOLINTBEGIN
// =============================================================================
// main.cpp — AsaMiia Interactive Client
// =============================================================================
...
/// Non-blocking read_key with timeout in milliseconds.
/// Returns KeyEvent with Key::UNKNOWN on timeout.
...
struct TerminalGuard {
    TerminalGuard()  { std::cout << c::ALT_ON  << c::HIDE_CURSOR << std::flush; }
    ~TerminalGuard() { std::cout << c::SHOW_CURSOR << c::ALT_OFF << std::flush; }
};

void clear() { std::cout << c::CLR << std::flush; }

std::string ask_text(const std::string& prompt) {
    std::cout << c::SHOW_CURSOR << c::B << "  " << prompt << c::RST << std::flush;
    ...
    tcsetattr(STDIN_FILENO, TCSANOW, &oldt);
    std::cout << c::HIDE_CURSOR << std::flush;
    return line;
}

int ask_int(const std::string& prompt, int fallback) {
    std::string raw = ask_text(prompt);
    if (raw.empty()) return fallback;
    try { return std::stoi(raw); } catch (...) { return fallback; }
}

void wait_key(const std::string& hint = "Press any key to continue...") {
    std::cout << "\n  " << c::DIM << hint << c::RST << std::flush;
    read_key();
}
...
void draw_header(const std::string& title, const std::string& subtitle = "") {
    std::cout << "\n " << c::BL << "┌─ " << c::RST << c::B << title << c::RST;
    if (!subtitle.empty()) std::cout << c::DIM << "  " << subtitle << c::RST;
    std::cout << "\n " << c::BL << "│" << c::RST << "\n";
}
...
void ui_ok(const std::string& m)  { std::cout << "  " << c::G   << "✓ " << c::RST << m << "\n"; }
void ui_err(const std::string& m) { std::cout << "  " << c::R   << "✗ " << c::RST << m << "\n"; }
void ui_wrn(const std::string& m) { std::cout << "  " << c::Y   << "⚠ " << c::RST << m << "\n"; }
void ui_inf(const std::string& m) { std::cout << "  " << c::DIM << "  " << m << c::RST << "\n"; }

// ============================================
// Interactive list selector (arrow navigation)
// ============================================
...
int list_selector(const std::string& title,
    ...
    while (true) {
        ...
        for (int i = 0; i < n; ++i) {
            if (i == sel)
                std::cout << "  " << c::CY << c::B << " ▸ " << options[i] << c::RST << "\n";
            else
                std::cout << "    " << c::DIM << options[i] << c::RST << "\n";
        }

        std::cout << "\n  " << c::DIM
                  << (footer_hint.empty() ? "↑↓ navigate  ENTER select  Q quit"
                                          : footer_hint)
                  << c::RST << "\n";
...
// ============================================
// Dummy input generator (for benchmark/warmup)
// ============================================

std::map<std::string, std::vector<float>> make_inputs(const ModelInfo& info) {
...
struct PerfStats {
    ...
    static double vec_pct(std::vector<double> v, double p) {
        ...
        // Nearest-rank (same formula as server-side)
        size_t n   = v.size();
    ...
    void print_table(double wall_ms = 0) const {
        if (empty()) {
            std::cout << "    " << c::R << "No successful samples." << c::RST << "\n";
            return;
        }
        ...
        printf("    %-22s %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f\n",
               ...
               vec_stddev(inference), vec_pct(inference, 0.50), vec_pct(inference, 0.99));

        if (!rtt.empty()) {
            printf("    %-22s %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f\n",
                   "Network RTT",
                   vec_avg(rtt), vec_min(rtt), vec_max(rtt),
                   vec_stddev(rtt), vec_pct(rtt, 0.50), vec_pct(rtt, 0.99));
        }

        std::cout << "\n";
        double total_inf = vec_sum(inference);
        if (total_inf > 0)
            printf("    Throughput : %.1f req/s (server-side)\n",
                   static_cast<double>(ok()) * 1000.0 / total_inf);
        if (wall_ms > 0) {
            printf("    Wall time  : %.1f ms (end-to-end)\n", wall_ms);
            printf("    Real rate  : %.1f req/s (including network)\n",
...
void record(const std::string& id, bool success, double inf_ms, double rtt_ms = 0) {
    session_stats[id].add(success, inf_ms, rtt_ms);
}
...
void flow_browse(Client& client) {
    auto available = client.list_available_models();
    if (available.empty()) {
        clear();
    ...
    if (chosen.is_loaded) {
        ...
        ui_inf("\"" + chosen.filename + "\" is loaded as \"" + chosen.loaded_as + "\"");
        draw_footer();
        wait_key();
        return;
    }
    ...
    for (const auto& s : v.inputs)  print_spec(s, "      ");
    for (const auto& s : v.outputs) print_spec(s, "      ");
    std::cout << "\n";
...
void flow_list(Client& client) {
    auto models = client.list_models();
    if (models.empty()) {
        clear(); draw_header("Loaded Models");
        ui_wrn("None loaded.");
        draw_footer();
        wait_key();
        return;
...
void flow_predict(Client& client) {
    ...
    if (inputs.empty()) { ui_err("Schema has no inputs"); wait_key(); return; }
    ...
    if (r.success) {
        ...
        for (const auto& [name, data] : r.outputs) {
            printf("    Out %-16s  %zu values  first=%.4f\n",
                   name.c_str(), data.size(), data.empty() ? 0.0f : data[0]);
        }
...
void flow_session_stats() {
    clear();
    draw_header("Session Statistics",
                "client-side measurements only");
    ...
    if (session_stats.size() > 1) {
        ...
        for (const auto& [_, s] : session_stats) {
            all.inference.insert(all.inference.end(), s.inference.begin(), s.inference.end());
...
// ============================================
// FLOW: Server Inference Metrics
//
// Obtém as métricas acumuladas directamente do worker via GetMetrics RPC.
// Cobre TODAS as origens: este cliente, outras aplicações, chamadas gRPC
// directas — tudo desde que o servidor arrancou.
//
// O ecrã actualiza-se automaticamente a cada 500 ms.
// Pressionar Q / ESC regressa ao menu principal.
// ============================================

static void render_server_metrics(const mlinference::client::ServerMetrics& m,
                                   int refresh_counter) {
    // Formata unix timestamp → "HH:MM:SS" (ou "—" se zero)
    ...
    // Clampa valores negativos — defensive guard para workers antigos
    // ou artefactos de serialização.  Após a correcção do model_backend.hpp
    // isto nunca deve ser accionado; mantém-se por robustez.
    auto clamp0 = [](double v) -> double { return v < 0.0 ? 0.0 : v; };

    clear();

    // Indicador de actualização (spinner simples)
    const char* spin[] = {"⠋","⠙","⠹","⠸","⠼","⠴","⠦","⠧","⠇","⠏"};
    ...
    // Worker summary
    std::cout << "    " << c::B << "Uptime      " << c::RST << m.uptime_seconds << " s\n"
              << "    " << c::B << "Requests    " << c::RST
    ...
    if (m.per_model.empty()) {
        ...
        for (const auto& [id, mm] : m.per_model) {
            ...
            if (ok_count > 0) {
                ...
                // Error rate (only when there are failures)
                ...
                if (mm.total_time_ms > 0)
                    printf("    Throughput  : %.1f req/s (server-side)\n",
                           static_cast<double>(ok_count) * 1000.0 / mm.total_time_ms);

                // Tail latency ratio — quick jitter indicator
...
void flow_server_metrics(Client& client) {
    ...
    // Renderiza imediatamente antes de esperar o primeiro timeout
    render_server_metrics(client.get_metrics(), counter++);

    while (true) {
        // Espera até 500 ms por input; regressa Key::UNKNOWN se não houver nada
        auto ev = poll_key(500);

        if (ev.key == Key::BACK || ev.key == Key::ESC)
            break;

        // Timeout (ou tecla ignorada) → actualiza
        render_server_metrics(client.get_metrics(), counter++);
    }
}
...
int main(int argc, char* argv[]) {
    std::string address = (argc > 1) ? argv[1] : "localhost:50052";

    // Connection phase (before alt screen)
    std::cout << c::CY << c::B
              << "\n"
              << "      _               __  __ _ _\n"
              << "     / \\   ___  __ _ |  \\/  (_|_) __ _\n"
    ...
    const std::vector<std::string> menu_items = {
        ...
        "Benchmark",
        "Worker status",
        "Session statistics",        // métricas deste cliente (RAM local)
        "Server inference metrics",  // métricas acumuladas no servidor
        "Quit",
    };
    ...
    while (true) {
        ...
        switch (ch) {
            case 0:  flow_browse(client);          break;
